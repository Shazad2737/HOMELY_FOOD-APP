  ...
  Future<Either<Failure, String>> forgotPassword({
    ...
    return resEither.fold(
      ...
      (response) async {
        ...
        if (body == null) {
          return left(
            AuthFailures.unknown('Unknown error occurred while requesting OTP'),
          );
        }
        ...
        try {
          ...
          log('Error parsing forgot password response: $e', stackTrace: s);
          return left(
            AuthFailures.unknown('Unknown error occurred while requesting OTP'),
          );
        }
  ...
  Future<Either<Failure, String>> resetPassword({
    ...
    return resEither.fold(
      ...
      (response) async {
        ...
        try {
          final message =
              body['message'] as String? ?? 'Password reset successfully';
          return right(message);
        } catch (e, s) {
          log('Error parsing password reset response: $e', stackTrace: s);
          return left(
            AuthFailures.unknown(
                'Unknown error occurred during password reset'),
          );
