...
import 'package:instamess_app/order_form/view/widgets/skipped_dates_warning_dialog.dart';
...
class DateChip extends StatelessWidget {
        ...
        return GestureDetector(
          onTap: () {
            ...
            if (state.selectedMealCount > 0 && state.selectedDate != null) {
              ...
              // Check for skipped dates before selecting
              _selectDateWithSkippedCheck(context, state);
            }
          },
          child: Container(
            width: 70,
  ...
  /// Check for skipped dates before selecting a date
  void _selectDateWithSkippedCheck(BuildContext context, OrderFormState state) {
    final skippedDates = state.getSkippedDates(day.date);

    if (skippedDates.isEmpty) {
      // No skipped dates, proceed with selection
      context.read<OrderFormBloc>().add(
        OrderFormDateSelectedEvent(day.date),
      );
    } else {
      // Show warning dialog about skipped dates
      final bloc = context.read<OrderFormBloc>();
      final targetDate = day.date;

      showDialog<void>(
        context: context,
        builder: (dialogContext) => SkippedDatesWarningDialog(
          skippedDates: skippedDates,
          onProceed: () {
            Navigator.pop(dialogContext);
            bloc.add(OrderFormDateSelectedEvent(targetDate));
          },
          onCancel: () {
            Navigator.pop(dialogContext);
          },
