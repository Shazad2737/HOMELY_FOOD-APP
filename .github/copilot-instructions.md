# Instamess App - AI Copilot Instructions
...
packages/
  ...
  └── instamess_api/   # API domain layer + session management
```

**Key principle**: Each package is self-contained. The main app primarily depends on `instamess_api` for domain facades, and also directly uses `core` (e.g., `DataState`, form inputs), and `app_ui` (components/tokens). Network calls must go through `instamess_api` facades — avoid using `ApiClient` directly in the app layer.
...
**SessionManager** (in `instamess_api`) is the single source of truth:
...
```dart
final api = InstaMessApi(baseUrl: 'http://localhost:4000'); // Dev example
...
7. Create corresponding repository method in `packages/instamess_api/`

### Adding a New API Endpoint

1. Add method to facade interface in `packages/instamess_api/lib/src/facades/`
...
| `packages/instamess_api/lib/instamess_api.dart` | Main API facade export                              |
...
The `SessionManager` (from `instamess_api`) provides:
